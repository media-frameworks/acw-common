...
export class HolodeckUtil {
   ...
   static compute_grid_vectors = (controls) => {
      ...
      const pov_direction = new Point3d(
         Math.cos(theta),
         Math.sin(theta),
         Math.tan(inclination)
      );
      const null_point = Point3d.nullPoint();
      const scaled_pov_vector = Vector3d.scale(
         Vector3d.normalize(
      ...
      const pov_vector = Vector3d.normalize(new Vector3d(null_point, pov_direction));
      console.log("pov_vector",pov_vector)
      const v_grid_vector = Vector3d.normalize(Vector3d.crossProduct(h_grid_vector, pov_vector));

      const grid_vectors = {
         pov_vector: scaled_pov_vector,
